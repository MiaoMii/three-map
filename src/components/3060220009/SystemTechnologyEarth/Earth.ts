...
import { createAnimateLine, createLightPillar, createPointMesh, createWaveMesh, getCirclePoints, lon2xyz } from "./common";
...
export default class earth {
    ...
    isLandByUV(c:number, f:number, earthImgData:any, earthImgWidth:number, earthImgHeight:number) {
        if (!earthImgData) {
            // 底图数据
            console.error('data error!');
        }
        const n = earthImgWidth * c; // 根据横纵百分比计算图象坐标系中的坐标

        const o = earthImgHeight * f; // 根据横纵百分比计算图象坐标系中的坐标

        return 100 < earthImgData.data[4 * (o * earthImgData.width + n)]; // 查找底图中对应像素点的rgba值并判断
        // return 0 === earthImgData.data[4 * (o * earthImgData.width + n)] // 查找底图中对应像素点的rgba值并判断
    }
    createEarthParticles(earthImgData:any, earthImgWidth:number, earthImgHeight:number) {

        const vertexShader = ['varying vec3 vVertexWorldPosition;',
        ...
        const fragmentShader = ['uniform vec3 glowColor;', 'uniform float  opacity;',
            ...
            ' if(intensity > division){ discard;}',
            ' intensity = intensity / division * scale;',
            ' intensity = pow(intensity, power);',
            ' gl_FragColor = vec4(glowColor, opacity*intensity);', '}'].join('\n');

        let positions:any = [];
        const spherical = new THREE.Spherical();
        spherical.radius = this.options.earth.radius;
        const step = 300;
        for (let i = 0; i < step; i++) {
            let vec:any = new THREE.Vector3();
            const _step = 1 / Math.cos(Math.abs(i - step / 2) / step * Math.PI);
            for (let j = 0; j < step; j += _step) {
                const c = j / step,
                    f = i / step
                if (this.isLandByUV(c, f, earthImgData, earthImgWidth, earthImgHeight)) {
       ...
       /* const pointsMaterial =new ShaderMaterial({
            uniforms: {
                ...
                color: {
                    value: new THREE.Color().setStyle(('#247ADA').toLocaleLowerCase())
                },
                pointTexture: {
                    value: new TextureLoader().load(getSrcPath('components/SystemComponents/30602/3060220009/SystemTechnologyEarth/images/earth/dot.png'))
                },
                size: {
                    value: 30
                },
                radius: {
                    value: this.options.earth.radius
                }
            },
            transparent: true,
            depthTest: false,
        ...
        const pointsMaterial = new THREE.PointsMaterial({
            size: 2,
            // map: this.textureLoader.load(window.appConfig.ASSETS_URL + defaultConfig.pointImage),
            color: new THREE.Color().setStyle(('#111111').toLocaleLowerCase()),
            blending: THREE.AdditiveBlending,
            // depthWrite: true,
            depthTest: false,
            transparent: true,
        ...
        console.log(points,'pointspointspointspointspointspoints', this.options.earth.radius)
    ...
    createEarth() {
        ...
        earthImg.setAttribute('crossOrigin', 'anonymous');
        earthImg.src = 'https://assets.easyv.cloud/components/static-image/earth3DNew/map_inverted3.jpg' // earthImg.src = window.appConfig.ASSETS_URL + 'components/earth3DNew/map_inverted.png';
        earthImg.onload = ()=> {
            const earthCanvas = document.createElement('canvas');
            const earthCtx:any = earthCanvas.getContext('2d');
            ...
            const earthImgData = earthCtx.getImageData(0, 0, earthImg.width, earthImg.height); // 根据图片添加点
            this.createEarthParticles(earthImgData, earthImg.width, earthImg.height);
        }
